...
- **[brain-system-prompt]**: the top-level brain system prompt is built by
  ...
  anthropic) gets "operator identity + harness instructions". It SKIPS Codex (that provider
...
- **[Codex-brain]**: `providers/Codex.py` (protocol `Codex`) drives the local
  ...
  `/profile Codex`. PRIMARY use is the TEXT brain: `complete()`/`complete_with_reasoning()`
  ...
  autonomous top-level loop, but that is BEST-EFFORT (~25% compliance): Codex's own agent
  ...
  different environment" framing - that reads as an injection to Codex and makes it MORE
  ...
  `Codex.asyncio.create_subprocess_exec` - never shell out to the real CLI in tests.
...
- **[sysprompts]**: the leaked product system-prompt corpus lives in `library/system_prompts/`
  ...
  matched on a single generic token ('does-not-exist-xyz' -> a random Codex file) because with
