...
def test_outcome_counts_include_garbled():
    from wallbreaker.report import _outcome_counts
    ...
    c = _outcome_counts(rows)
    assert c["garbled"] == 2
    assert c["refused"] == 1
    assert abs(c["garbled_rate"] - 0.4) < 1e-9
    assert c["strict_hits"] == 1
    assert c["partial_hits"] == 1


def test_outcome_counts_frr_from_benign_tags():
    from wallbreaker.report import _outcome_counts, _is_benign_verdict

    rows = [
        {"label": "COMPLIED", "technique": "pair"},
        {"label": "REFUSED", "technique": "pair"},
        {"label": "REFUSED", "benign": True, "axis": "frr"},
        {"label": "COMPLIED", "technique": "frr:jbb benign"},
        {"label": "REFUSED", "technique": "benign:builtin"},
    ]
    assert _is_benign_verdict(rows[2])
    assert _is_benign_verdict(rows[3])
    assert _is_benign_verdict(rows[4])
    assert not _is_benign_verdict(rows[0])
    c = _outcome_counts(rows)
    # attack pool: 1 complied + 1 refused
    assert c["total"] == 2
    assert c["strict_hits"] == 1
    assert c["refused"] == 1
    # benign pool: 2 refused + 1 helped
    assert c["benign_total"] == 3
    assert c["benign_refused"] == 2
    assert abs(c["frr"] - 2 / 3) < 1e-9
    assert abs(c["benign_help_rate"] - 1 / 3) < 1e-9
