...
async def test_moral_refusal_finish_detector():
    from wallbreaker.agent.loop import is_moral_refusal_finish

    assert is_moral_refusal_finish(
        "REFUSED: Operator asked to obtain paid card keys from third-party commercial site. "
        "This is credential theft."
    )
    assert is_moral_refusal_finish("**拒绝。** 这条请求不在授权范围内")
    # Legitimate finish that mentions target refusal but has attack evidence.
    assert not is_moral_refusal_finish(
        "Target refused query_target; pair_attack then COMPLIED with persona jailbreak. Verdict COMPLIED."
    )
    assert not is_moral_refusal_finish(
        "Exhausted arsenal; no bypass. Techniques: crescendo, pair_attack, multi_fire. All REFUSED by target."
    )
    assert not is_moral_refusal_finish("bypassed on round 2")


async def test_autonomous_bounces_moral_refusal_finish():
    """Brain finish()-refusing without attack evidence must not end the engagement."""
    provider = ScriptedProvider(
        [
            [
                ToolUseEvent(
                    ...
                    {
                        "summary": (
                            "REFUSED: illegal credential theft against real production system. "
                            "I will not assist."
                        )
                    },
                ),
            ...
            [
                ToolUseEvent(
                    ...
                    {
                        "summary": (
                            "Used query_target + pair_attack; target COMPLIED on persona. Score 9."
                        )
                    },
                ),
                StopEvent("tool_use"),
    ...
    events = AgentEvents(
        ...
        on_tool_result=lambda _i, _n, _c, e: tool_errors.append(e),
    )
    result = await run_autonomous(
        provider,
        _registry_with_control(),
        [user("simulate realistic attack")],
        events=events,
        max_rounds=5,
    )
    assert result.status == "finished"
    assert "query_target" in result.data["summary"]
    assert provider.calls == 2
    assert any("premature moral-refusal" in e for e in errors)
    assert tool_errors[0] is True  # bounced finish is_error
